pipeline {
    ...
    stages {
        ...
        stage('Maven Compile & Package') {
            steps {
                echo "[BUILD] Compiling Java Web Application..."
        ...
        stage('AWS ECR Login & Authenticate') {
            steps {
                echo "[AUTH] Logging into Amazon ECR..."
        ...
        stage('Deploy to Amazon EKS') {
            ...
            steps {
                ...
                withCredentials([
                    ...
                    sed -i "s|image: .*|image: ${LOCAL_ECR_URL}/${TARGET_SERVICE}:${BUILD_NUMBER}|g" k8s-deploy.yml
                    ...
                    docker run --rm \
                      ...
                      apply -f k8s-deploy.yml -n production
                    ...
                    docker run --rm \
                      ...
                      -w /apps \
                      bitnami/kubectl:latest \
                      rollout status deployment/mavenwebappdeployment \
                      -n production \
                      --timeout=180s
